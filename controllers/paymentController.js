...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    await db.query(
      `UPDATE payments SET status='paid', verified_at=NOW(), verified_by_admin=? WHERE order_id=?`,
      [req.user.mysqlId, orderId]
    );
...
exports.getUserPaymentStatus = async (req, res) => {
  try {
    ...
    const [rows] = await db.query(
      `SELECT status, order_id, utr, screenshot, created_at, submitted_at, verified_at 
       FROM payments 
       WHERE booking_id = ?
